  ...
  try {
    ...
    for (requiredData of ["username", "password", "email"]){
        ...
        for(x of data[requiredData]){
            if(["--", "_", "-", "%", "'", '"', "$", "&", "`", "#", "*"].indexOf(x) != -1)return res.status(400).json({status_code:400, message: (req.isIndo)? `Simbol ${x} Tidak Bisa di Gunakan!` : `Can't Use ${x} Symbol for Input!`});
        }
    }
    
    // Username Min 5 Char
    if (data.username.length < 5) return res.status(400).json({status_code:400, message: (req.isIndo)? `Username Minimal 5 Karakter!` : `Username Min 5 Character!`});
        
    // Username Unique
    const usernameExistDoc = await User.where("username", "==", data.username).where("is_verified", "==", true).get();
    if (!(usernameExistDoc.empty)) return res.status(400).json({status_code:400, message: (req.isIndo)? `Username Sudah Digunakan!` : `Username Already Used!`}); 
    
    // Password Min 8
    if (data.password.length < 8) return res.status(400).json({status_code:400, message: (req.isIndo)? `Password Minimal 8 Karakter!` : `Password Min 8 Character!`});
    
    // Check Email
    if(!validator.isEmail(data.email)) return res.status(400).json({status_code:400, message: (req.isIndo)? `Gunakan Format Email yang Valid/Benar!` : `Use Valid Email Format!`});

    // Email Unique
    const emailExistDoc = await User.where("email", "==", data.email).where("is_verified", "==", true).get();
    if (!(emailExistDoc.empty)) return res.status(400).json({status_code:400, message: (req.isIndo)? `Email Sudah Digunakan!` : `Email Already Used!`})
...
exports.updateProfile = () => async (req, res, next) => {
  try { 
    ...
    // Check Data in Body
    if ((req.body == null || Object.keys(req.body) == 0) && (!req.image_url || req.image_url == null)) return res.status(400).json({status_code:400, message: (req.isIndo)? `Tidak Ada Data yang Perlu di Update!` : `No Data To Be Updated!`});
    ...
    if (data.new_password){
        ...
        // Password Min 8
        if (data.new_password.length < 8) return res.status(400).json({status_code:400, message: (req.isIndo)? `Password Minimal 8 Karakter!` : `Password Min 8 Character!`});        
    }
    
    // Delete Object or Emptyed it If Field Inside it Random
    if (!("username" in data) && !("email" in data) && !("new_password" in data) && !("name" in data) && !("image_url" in data)){
        data = null;
    }
